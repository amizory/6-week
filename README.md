...
->  Чтобы выполнить задачи из плейбука от привилегированного пользователя, можно использовать
    модуль become или sudo. Это позволяет выполнить команды с повышенными привилегиями,
...
- name: Обновить пакеты
  ...
  apt:
    name: "*"
    state: lates
    В этом примере модуль apt будет выполнен с привилегиями пользователя root, что
    позволит обновить пакеты
...
- name: Обновить пакеты
  ...
  apt:
    name: "*"
    state: latest
    Оба способа позволяют выполнить команды с повышенными привилегиями, но become является
    более рекомендуемым и современным способом
```
...
->  - sudo: Использует команду sudo для эскалации привилегий. Это наиболее распространенный
    метод, который позволяет выполнить команды с привилегиями суперпользователя.
    - su: Использует команду su для эскалации привилегий. Этот метод требует, чтобы пароль
    суперпользователя был указан явно.
    - pfexec: Использует команду pfexec для эскалации привилегий (поддерживается только на Solaris).
    - doas: Использует команду doas для эскалации привилегий (поддерживается только на OpenBSD).
    - dzdo: Использует команду dzdo для эскалации привилегий (поддерживается только на Debian).
    - ksu: Использует команду ksu для эскалации привилегий (поддерживается только на AIX).
    - runas: Использует команду runas для эскалации привилегий (поддерживается только на Windows).
  ...
  - name: Обновить пакеты
    ...
    - name: Обновить пакеты
      ...
      become_method: su
      become_user: root
      become_pass: пароль_суперпользователя
      apt:
        name: "*"
        state: latest
    Можно также указать метод эскалации привилегий в файле конфигурации Ansible
    (ansible.cfg) с помощью параметра become_method. Например:
    [defaults]
    become_method = sudo
```
...
->  - Handler (обработчик) в Ansible - это специальный тип задачи, который выполняется только
    ...
    ---
- name: Обновить конфигурацию веб-сервера
  hosts: web_servers
  tasks:
  ...
  - name: restart_nginx
    ...
    В этом примере, после того, как конфигурация веб-сервера будет обновлена, handler
    restart_nginx будет выполнен, который перезапустит сервис nginx.

    Handler можно вызвать с помощью ключевого слова notify в задаче, которая изменяет ресурс
    или переменную. Handler будет выполнен только в том случае, если была изменена какая-либо
    переменная или ресурс.
    Также handler можно вызвать с помощью ключевого слова listen в handler, который будет
    слушать изменения ресурсов или переменных. Например:
...
- name: Обновить конфигурацию веб-сервера
  ...
  - name: restart_nginx
    ...
    В этом примере, handler restart_nginx будет слушать изменения конфигурации веб-сервера
    и будет выполнен только в том случае, если конфигурация будет изменена.
```

```txt
3 - Что происходит на этапе Gathering Facts при выполнении плейбука?

->  На этапе Gathering Facts (сбор фактов) при выполнении плейбука Ansible собирает информацию
    о целевых хостах, на которых будет выполняться плейбук. Этот этап также известен как
    "факт-сбор" или "инвентаризация".

    Во время этого этапа Ansible выполняет следующие действия:
    - Сбор информации о хостах: Ansible собирает информацию о каждом хосте, указанном в
    инвентаре (inventory), включая его IP-адрес, имя хоста, операционную систему, версию и
    другую информацию.
    - Определение переменных: Ansible определяет переменные, которые будут использоваться в
    плейбуке, такие как ansible_distribution, ansible_os_family, ansible_architecture и другие.
    - Сбор информации о сетевых интерфейсах: Ansible собирает информацию о сетевых интерфейсах
    хоста, включая IP-адреса, маски подсети, шлюзы и другие параметры.
    - Сбор информации о дисковых устройствах: Ansible собирает информацию о дисковых
    устройствах хоста, включая тип устройства, размер, файловую систему и другие параметры.
    - Сбор информации о процессоре: Ansible собирает информацию о процессоре хоста, включая
    тип процессора, количество ядер, частоту и другие параметры.

    Собранная информация хранится в виде переменных, которые можно использовать в плейбуке для
    принятия решений или выполнения задач. Эти переменные можно использовать в шаблонах,
    задачах и других элементах плейбука.
    Например, если вы хотите использовать переменную ansible_distribution для установки пакета,
    который зависит от операционной системы, вы можете использовать следующий код:
    ---
- name: Установить пакет
  apt:
    name: "{{ ansible_distribution }}-package"
    state: present
    В этом примере Ansible будет использовать переменную ansible_distribution для определения
    операционной системы хоста и установки соответствующего пакета.
```

```txt
4 - Как можно узнать имя и версию дистрибутива Linux, на котором в данных момент выполнятся плейбук?

->  В Ansible можно узнать имя и версию дистрибутива Linux, на котором в данный момент
    выполняется плейбук, используя переменные ansible_distribution и ansible_distribution_version

    Эти переменные собираются во время этапа Gathering Facts (сбор фактов) и содержат информацию
    о дистрибутиве Linux, на котором выполняется плейбук.
    Например, вы можете использовать следующий код для вывода имени и версии дистрибутива Linux:

    ---
- name: Вывести информацию о дистрибутиве
  debug:
    msg: "Дистрибутив: {{ ansible_distribution }}, Версия: {{ ansible_distribution_version }}"
    В этом примере Ansible будет выводить сообщение с именем и версией дистрибутива Linux, на
    котором выполняется плейбук

    Вы также можете использовать эти переменные в условных операторах или в шаблонах для
    принятия решений или выполнения задач в зависимости от дистрибутива Linux.
    Например, вы можете использовать следующий код для установки пакета, который зависит от
    дистрибутива Linux:
...
- name: Установить пакет
  ...
  В этом примере Ansible будет устанавливать пакет только на хостах с дистрибутивом Ubuntu.

  Или вы можете использовать следующий код для выполнения задачи только на хостах с
  определенной версией дистрибутива Linux:
    ---
- name: Выполнить задачу
  shell: "echo 'Выполнено!'"
  when: ansible_distribution_version == "18.04"
  В этом примере Ansible будет выполнять задачу только на хостах с версией дистрибутива Linux 18.04.
```

```txt
5 - Можно ли пропустить этап Gathering Facts ?

->  - Да, можно пропустить этап Gathering Facts в Ansible.
    - По умолчанию, Ansible выполняет этап Gathering Facts перед выполнением плейбука, чтобы
    собрать информацию о целевых хостах. Однако, если вы не нуждаетесь в этой информации или
    если вы хотите ускорить выполнение плейбука, вы можете пропустить этот этап.
    - Для пропуска этапа Gathering Facts можно использовать ключ --no-gather-facts при запуске
    плейбука:
    ansible-playbook -i inventory playbook.yml --no-gather-facts
    Альтернативно, можно добавить директиву gather_facts: no в начало плейбука:
        ---
    - name: Плейбук
      gather_facts: no
      hosts: ...
    ...
    Пропуск этапа Gathering Facts может быть полезен в следующих случаях:
    - Если вы не нуждаетесь в информации о целевых хостах.
    - Если вы хотите ускорить выполнение плейбука.
    - Если вы выполняете плейбук на большом количестве хостов и хотите сократить время выполнения.
    Однако, если вы пропустите этап Gathering Facts, вы не сможете использовать переменные,
    которые собираются во время этого этапа, такие как ansible_distribution,
    ansible_os_family и другие.
```

```txt
6 - Как найти и заменить строку в заданном файле с помощью регулярного выражения?

->  В Ansible можно найти и заменить строку в заданном файле с помощью регулярного выражения
  используя модуль replace.
  Например, чтобы найти и заменить строку в файле /etc/hosts с помощью регулярного выражения,
  можно использовать следующий код
    ---
- name: Найти и заменить строку в файле
  replace:
    path: /etc/hosts
    regexp: '^127\.0\.0\.1\s+localhost$'
    replace: '127.0.0.1 mylocalhost'

    В этом примере Ansible будет искать строку, которая начинается с 127.0.0.1 и
    содержит слово localhost, и заменять ее на строку 127.0.0.1 mylocalhost.
    Модуль replace также поддерживает следующие опции:
    - path: путь к файлу, в котором нужно найти и заменить строку.
    - regexp: регулярное выражение, которое нужно использовать для поиска строки.
    - replace: строка, которая будет использована для замены найденной строки.
    - backup: если установлено значение yes, то Ansible будет создавать резервную копию
    файла перед заменой строки.

    Также можно использовать модуль lineinfile для добавления или замены строки в файле.
    ---
- name: Добавить или заменить строку в файле
  lineinfile:
    path: /etc/hosts
    regexp: '^127\.0\.0\.1\s+localhost$'
    line: '127.0.0.1 mylocalhost'
    state: present
    В этом примере Ansible будет искать строку, которая начинается с 127.0.0.1 и содержит
    слово localhost, и заменять ее на строку 127.0.0.1 mylocalhost, если строка найдена, или
    добавлять строку 127.0.0.1 mylocalhost, если строка не найдена.
```

```txt
7 - Как зарегистрировать переменную в плейбуке в runtime?

->  В Ansible можно зарегистрировать переменную в плейбуке в runtime с помощью ключевого
    слова register.
    Например, чтобы зарегистрировать переменную my_variable и присвоить ей значение результата
    выполнения команды ls -l, можно использовать следующий код:
    ---
- name: Зарегистрировать переменную
  shell: ls -l
  register: my_variable
    Зарегистрированную переменную можно использовать в дальнейшем в плейбуке,
    например, для вывода ее значения:

    ---
- name: Вывести значение переменной
  debug:
    msg: "Значение переменной: {{ my_variable.stdout }}"

    Можно также зарегистрировать переменную и присвоить ей значение результата выполнения
    модуля Ansible. Например, чтобы зарегистрировать переменную my_variable и присвоить ей
    значение результата выполнения модуля stat, можно использовать следующий код:
    ---
- name: Зарегистрировать переменную
  stat:
    path: /etc/hosts
...
- name: Вывести значение переменной
  debug:
    msg: "Значение переменной: {{ my_variable.stat.exists }}"

    Также можно зарегистрировать переменную и присвоить ей значение результата выполнения
    блока кода. Например, чтобы зарегистрировать переменную my_variable и присвоить ей
    значение результата выполнения блока кода, можно использовать следующий код:
    ---
- name: Зарегистрировать переменную
  block:
    - shell: ls -l
...
->  Ansible-Galaxy - это репозиторий ролей Ansible, который позволяет пользователям легко
    найти, скачать и использовать роли Ansible, созданные другими пользователями.

    Ansible-Galaxy предоставляет следующие возможности:
    - Поиск ролей: Ansible-Galaxy позволяет пользователям искать роли Ansible по ключевым
    словам, тегам и описаниям.
    - Скачивание ролей: Пользователи могут скачать роли Ansible из Ansible-Galaxy и
    использовать их в своих плейбуках.
    - Публикация ролей: Пользователи могут публиковать свои собственные роли Ansible в
    Ansible-Galaxy, чтобы другие пользователи могли их использовать.
    - Управление зависимостями: Ansible-Galaxy позволяет пользователям управлять зависимостями
      между ролями Ansible, что упрощает процесс использования ролей в плейбуках.
    
    Ansible-Galaxy предоставляет следующие преимущества:
    - Упрощение процесса использования ролей: Ansible-Galaxy упрощает процесс использования
    ролей Ansible, позволяя пользователям легко найти и скачать роли, которые они cần.
    - Сообщество: Ansible-Galaxy предоставляет платформу для сообщества пользователей Ansible,
    где они могут делиться своими ролями и получать обратную связь от других пользователей.
    - Качество ролей: Ansible-Galaxy позволяет пользователям оценить качество ролей Ansible,
    что помогает другим пользователям принимать обоснованные решения о том, какие роли
    использовать.

    Чтобы использовать Ansible-Galaxy, необходимо выполнить следующие шаги:
    - Установить Ansible-Galaxy: Ansible-Galaxy можно установить с помощью команды
    ansible-galaxy install.
    - Поиск ролей: Пользователи могут искать роли Ansible в Ansible-Galaxy с помощью
    команды ansible-galaxy search.
    - Скачивание ролей: Пользователи могут скачать роли Ansible из Ansible-Galaxy с
    помощью команды ansible-galaxy install.
    - Использование ролей: Пользователи могут использовать скачанные роли Ansible в
    своих плейбуках.
    ...
    - name: Установить и настроить Nginx
      ...
      roles:
        - geerlingguy.nginx
    В этом примере мы устанавливаем Ansible-Galaxy, ищем роль "nginx",
    скачиваем роль "geerlingguy.nginx" и используем ее в плейбуке для установки и
    настройки Nginx на веб-серверах.
```

```txt
9 - Как установить дополнительный модуль/плагин из Ansible-galaxy?

->  Чтобы установить дополнительный модуль/плагин из Ansible-Galaxy, необходимо выполнить
    следующие шаги:
    - Поиск модуля/плагина: Используйте команду ansible-galaxy search для поиска модуля/плагина,
    который вы хотите установить.
    ansible-galaxy search <имя_модуля>
    - Установка модуля/плагина: Используйте команду ansible-galaxy install для установки
    модуля/плагина.
    ansible-galaxy install <имя_модуля>
    - Проверка установки: Используйте команду ansible-galaxy list для проверки установки
    модуля/плагина.
    ansible-galaxy list

    # Поиск модуля
    ansible-galaxy search geerlingguy.docker
    # Установка модуля
    ansible-galaxy install geerlingguy.docker
    # Проверка установки
    ansible-galaxy list
    Также вы можете установить модуль/плагин из Ansible-Galaxy с помощью файла
    requirements.yml. Для этого необходимо создать файл requirements.yml с содержимым:
    ---
    roles:
      - geerlingguy.docker
    Затем вы можете установить модуль/плагин с помощью команды:
    ansible-galaxy install -r requirements.yml

    После установки модуля/плагина вы можете использовать его в своих плейбуках. Например:
    ---
...
->  Чтобы найти все файлы в заданной директории, которые оканчиваются на .log и записать их
    полные пути в переменную в плейбуке, вы можете использовать модуль find в Ansible.
    ---
    - name: Найти все файлы .log в директории
      hosts: localhost
      gather_facts: no

      tasks:
      - name: Найти все файлы .log
        find:
          paths: /var/log
          patterns: '*.log'
        register: log_files

      - name: Вывести полные пути файлов
        debug:
          msg: "{{ log_files.files | map(attribute='path') | list }}"
      В этом примере мы используем модуль find для поиска всех файлов в директории /var/log,
      которые оканчиваются на .log. Результаты поиска записываются в переменную log_files.
      Затем мы используем модуль debug для вывода полных путей файлов, которые были найдены.
      Мы используем фильтр map для извлечения атрибута path из каждого файла и фильтр list
      для преобразования результата в список.

      Если вы хотите записать полные пути файлов в переменную, вы можете использовать
      модуль set_fact вместо debug
        - name: Записать полные пути файлов в переменную
          set_fact:
...
->  Плагин lookup в Ansible позволяет вам получить доступ к различным источникам данных и
    использовать их в ваших плейбуках. С помощью плагина lookup вы можете:
    - Читать файлы: Вы можете читать содержимое файлов и использовать его в ваших плейбуках.
    lookup('file', '/path/to/file')
    - Читать переменные окружения: Вы можете читать переменные окружения и использовать их в
    ваших плейбуках. lookup('env', 'VARIABLE_NAME')
    - Читать данные из базы данных: Вы можете читать данные из базы данных и использовать их в
    ваших плейбуках. lookup('mysql', 'SELECT * FROM table_name')
    - Читать данные из API: Вы можете читать данные из API и использовать их в ваших плейбуках.
    lookup('url', 'https://api.example.com/data')
    - Читать данные из файла CSV: Вы можете читать данные из файла CSV и использовать их в
    ваших плейбуках. lookup('csvfile', '/path/to/file.csv')
    - Читать данные из файла JSON: Вы можете читать данные из файла JSON и использовать их в
    ваших плейбуках. lookup('jsonfile', '/path/to/file.json')
    - Читать данные из файла YAML: Вы можете читать данные из файла YAML и использовать их в
    ваших плейбуках. lookup('yamlfile', '/path/to/file.yaml')
    - Читать данные из файла INI: Вы можете читать данные из файла INI и использовать их в
    ваших плейбуках. lookup('inifile', '/path/to/file.ini')

          ---
    - name: Читать данные из базы данных PostgreSQL
      hosts: localhost
      gather_facts: no

      tasks:
      - name: Читать данные из базы данных
        lookup:
          plugin: postgresql
          db: mydatabase
          table: mytable
...
->  - Параллельное выполнение: Ansible позволяет выполнять задачи параллельно на нескольких
    хостах. Это можно сделать с помощью ключа -f или --forks при запуске плейбука.
    ansible-playbook -f 10 playbook.yml
    Это позволит Ansible выполнять задачи на 10 хостах параллельно.

    - Использование модуля async: Модуль async позволяет выполнять задачи асинхронно, что
    может ускорить выполнение плейбука.
        ---
    - name: Выполнить задачу асинхронно
      async:
        name: my_task
        wait: 10
    
    - Использование модуля delegate_to: Модуль delegate_to позволяет делегировать выполнение
    задачи на другой хост.
        ---
    - name: Выполнить задачу на другом хосте
      delegate_to: another_host
      task: my_task

    - Использование модуля poll: Модуль poll позволяет периодически проверять статус задачи и
    завершать ее, если она выполнена.
        ---
    - name: Проверить статус задачи
      poll:
        name: my_task
        interval: 10
        timeout: 60
    
    - Оптимизация плейбука: Оптимизация плейбука может включать в себя удаление ненужных
    задач, объединение задач и использование более эффективных модулей.
        ---
    - name: Оптимизировать плейбук
      tasks:
    ...
    - Использование модуля strategy: Модуль strategy позволяет указать стратегию выполнения
    задач.
    ---
    - name: Указать стратегию выполнения задач
      strategy:
        name: my_strategy
        tasks:
        - name: Выполнить задачу 1
    ...
    - Использование модуля cache: Модуль cache позволяет кэшировать результаты задач, чтобы
    избежать повторного выполнения одних и тех же задач.
    ---
  - name: Кэшировать результаты задач
    cache:
      name: my_cache
      tasks:
      - name: Выполнить задачу 1
